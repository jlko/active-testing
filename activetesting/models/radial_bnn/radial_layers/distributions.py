...
def scale_mixture_prior(name,
                        log2pi,
                        mus,
                        sigmas,
                        pi,
                        device):
    first_log_gaussian_pdf = gaussian_prior(name, log2pi, mus[0], sigmas[0], device)
    second_log_gaussian_pdf = gaussian_prior(name, log2pi, mus[1], sigmas[1], device)

    def log_mixture_pdf(x):
        return pi * first_log_gaussian_pdf(x) + (1 - pi) * second_log_gaussian_pdf(x)
